...
class Chat(Object):
    ...
    def _parse_channel_chat(client, channel: raw.types.Channel) -> Optional["Chat"]:
        ...
        subscription_until_date = (
            utils.timestamp_to_datetime(subscription_until)
    ...
    async def _parse_full(
        ...
        if isinstance(chat_full, raw.types.users.UserFull):
            ...
            if isinstance(full_chat, raw.types.ChatFull):
                ...
                parsed_chat.is_paid_reactions_available = (
                    full_chat.paid_reactions_available
                )
                parsed_chat.is_gifts_available = getattr(
                    full_chat, "stargifts_available", None
                )
            ...
            parsed_chat.bot_verification = types.BotVerification._parse(
                client, getattr(full_chat, "bot_verification", None), users
            )
    ...
    async def promote_member(
        ...
        return await self._client.promote_chat_member(
            chat_id=self.id, user_id=user_id, privileges=privileges, title=title
        )
